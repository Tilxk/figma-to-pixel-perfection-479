...
const Button: React.FC<ButtonProps & React.ButtonHTMLAttributes<HTMLButtonElement>> = ({ 
  ...
  const variants = {
    primary: 'bg-[linear-gradient(173deg,#d6e4ff_0%,#000000_50%,#d4e4ff_100%)] text-button-2 hover:opacity-90 active:opacity-80 focus:ring-blue-500',
  ...
  const responsiveRadius = 'rounded-[22px]';
  ...
  return (
    <button
      ...
      className={`
        ...
        touch-manipulation
        shadow-[0px_4px_16px_#00000033]
        gap-[10px]
        pt-[12px] pr-[22px] pb-[12px] pl-[50px]
        ${className}
      `.trim().replace(/\s+/g, ' ')}
